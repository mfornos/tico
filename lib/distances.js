...
class Distances {
   ...
   * @return {number} d The Canberra distance between two terms
   */
  static canberra(a, b) {
    var sum = 0;
    for(var i = 0; i < a.length; i++) {
      sum += Distances.abs(a[i], b[i]) /
   ...
   * @return {number} d The absolute diference between two terms
   */
  static abs(a, b) {
    return Math.abs(a - b);
  }
  ...
  static manhattan(a, b) {
    return a.map( (x, i) => Distances.abs(x, b[i]) )
            .reduce( (p, x) => p + x );
  }
   ...
   * @param  {Object} a The first term
   * @param  {Object} b The second term
   * @return {number} d The hamming distance
   */
  static hamming(a, b) {
    if(Array.isArray(a)) {
      let sum = 0;
      for(var i = 0; i < a.length; i++) {
        sum += Distances.hamming(a[i], b[i]);
      }
      return sum;
   ...
   * @param  {Array}  a The first term
   * @param  {Array}  b The second term
   * @return {number} d The SM distance between two terms
   */
  static SMD(a, b) {
